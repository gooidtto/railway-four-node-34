...
def expected_nodes():
    try:
        n = int(json.loads(RUNTIME.read_text()).get("nodes", {}).get("count", 0)); return n if n in (4, 5) else 0
    except Exception: return 0

def local_port_ready(port):
    try:
        with socket.create_connection(("127.0.0.1", port), timeout=1.5): return True
    except OSError: return False

def cloudflare_ready():
    try:
        cf = json.loads(RUNTIME.read_text()).get("cloudflare", {})
        if not cf.get("enabled"): return True
...
def subscription(token, request_host=""):
    ...
    # Railway-generated public domains can change between deployments while
    # a persistent volume still contains the previous runtime snapshot.
    # For the subscription fetched from the current public host, rewrite only
    # Node 1's endpoint/SNI to the actual Host header. TCP Proxy nodes remain
    # sourced from the current Railway networking environment at generation.
    host = (request_host or "").strip().split(",", 1)[0].strip().lower()
    if host and ":" in host and not host.startswith("["):
        host = host.rsplit(":", 1)[0]
    if host and re.fullmatch(r"[a-z0-9.-]+", host):
        try:
            u = urllib.parse.urlsplit(lines[0])
            if u.scheme == "vless" and u.hostname:
                q = urllib.parse.parse_qsl(u.query, keep_blank_values=True)
                q = [(k, host if k == "sni" else v) for k, v in q]
                lines[0] = urllib.parse.urlunsplit((u.scheme, f"{u.username}@{host}:443" if u.username else f"{host}:443", u.path, urllib.parse.urlencode(q), u.fragment))
        except Exception as exc:
            log.warning("SUBSCRIPTION_HOST_REWRITE_FAILED host=%s error=%s:%s", host, type(exc).__name__, exc)
...
async def read_initial(reader):
    ...
    while len(buf) < MAX_INITIAL:
        ...
        if b.startswith(HTTP):
            ...
            early_sni = tls_sni(b)
            if early_sni:
                log.warning("TLS_SNI_EARLY sni=%s initial=%d", early_sni, len(b)); return b
...
async def relay(reader, writer, initial, dest, label, sni="-"):
    ...
    try:
        ...
        if initial:
            up.write(initial); await up.drain(); log.warning("INITIAL_FORWARDED route=%s bytes=%d", label, len(initial))
...
async def http(reader,writer,initial):
    ...
    if method in ("GET","HEAD") and m:
        host=urllib.parse.unquote(next((v for k,v in re.findall(r"(?i)^host:\s*([^\r\n]+)", initial.decode("latin1","ignore"), re.M)), "")) if False else ""
        header_host=""
        for hline in initial.decode("latin1","ignore").split("\r\n"):
            if hline.lower().startswith("host:"):
                header_host=hline.split(":",1)[1].strip(); break
        payload,status=subscription(urllib.parse.unquote(m.group(1)),header_host)
...
async def handle(reader,writer):
    peer=writer.get_extra_info("peername"); log.warning("TCP_ACCEPT peer=%s local=%s",peer,writer.get_extra_info("sockname"))
    async with SEM:
        try:
            ...
            if initial.startswith(HTTP): log.warning("PROTOCOL_DETECTED peer=%s protocol=http",peer); await http(reader,writer,initial); return
            if initial[:1]==b"\x16" and len(initial)>=3 and initial[1]==0x03:
                sni=tls_sni(initial); log.warning("TLS_SNI peer=%s sni=%s initial=%d",peer,sni or "-",len(initial)); route=ROUTES.get(sni or "")
                if route: log.warning("ROUTE_MATCH peer=%s sni=%s route=%s dest=%s:%s",peer,sni,route[2],route[0],route[1]); await relay(reader,writer,initial,(route[0],route[1]),route[2],sni); return
...
async def main():
    server=await asyncio.start_server(handle,"0.0.0.0",PORT,limit=262144); log.warning("GATEWAY_READY=%s max_connections=%s idle_timeout=%ss",PORT,MAX_CONNECTIONS,IDLE_TIMEOUT); log.warning("ROUTES=%s",",".join(f"{k}->{v[1]}" for k,v in ROUTES.items())); await server.serve_forever()
