...
if [ "$CF_ENABLED" = 1 ]; then [ -n "$CF_HOST_STATE" ] && [ -n "$CF_ORIGIN_STATE" ] && [ -n "$CF_PORT_STATE" ] && [ -n "$CF_PATH_STATE" ] || { echo "FATAL: runtime state says Cloudflare enabled but required state is incomplete" >&2; exit 1; }; [ -n "$CF_TOKEN" ] || { echo "FATAL: runtime state says Cloudflare enabled but tunnel token is unavailable" >&2; exit 1; }; fi
python3 - "$C" "$D/subscription.txt" "$UUID" <<'PY'
import json,re,sys
from pathlib import Path
config=json.loads(Path(sys.argv[1]).read_text()); sub=Path(sys.argv[2]).read_text().splitlines(); expected=sys.argv[3]
config_ids=[client.get("id") for inbound in config.get("inbounds",[]) for client in inbound.get("settings",{}).get("clients",[]) if client.get("id")]
if not config_ids or any(x != expected for x in config_ids): raise SystemExit("FATAL: Xray UUID does not match current deployment UUID")
for i,line in enumerate(sub,1):
    if not line.strip(): continue
    m=re.match(r"vless://([^@]+)@",line.strip())
    if not m or m.group(1)!=expected: raise SystemExit(f"FATAL: subscription UUID mismatch at node {i}")
print(f"UUID_INVARIANT=OK uuid={expected}")
PY
python3 - "$D/subscription.txt" "$RUNTIME" <<'PY'
import json,re,sys
from pathlib import Path
lines=[x.strip() for x in Path(sys.argv[1]).read_text().splitlines() if x.strip()]; runtime=json.loads(Path(sys.argv[2]).read_text()); expected=int(runtime["nodes"]["count"]); public=runtime["public_domain"]; tcp=runtime["tcp_proxy"]
if len(lines)!=expected: raise SystemExit(f"FATAL: expected {expected} nodes, got {len(lines)}")
if not re.match(rf"^vless://[^@]+@{re.escape(public)}:443\?", lines[0]): raise SystemExit("FATAL: NODE1 endpoint does not match current Railway public domain")
for idx in (1,2,3):
    if not re.match(rf"^vless://[^@]+@{re.escape(tcp['domain'])}:{int(tcp['port'])}\?", lines[idx]): raise SystemExit(f"FATAL: NODE{idx+1} endpoint does not match current Railway TCP proxy")
...
echo "RELEASE=$BUILD_ID"; echo "ARCHITECTURE=fixed-node-order-1-railway-xhttp-2-raw-reality-3-xhttp-reality-4-grpc-reality-5-cloudflare-ws"; echo "NODE_ORDER=1:railway-xhttp-tls,2:raw-reality-vision,3:xhttp-reality,4:grpc-reality,5:cloudflare-ws-tls"; echo "TARGET_PORT=8080"; echo "TCP=$TCP_HOST:$TCP_PORT -> 8080"; echo "ROUTES=HTTP->10086,RAW-REALITY->10087,XHTTP-REALITY->10088,GRPC-REALITY->10089"; [ "$CF_ENABLED" = 1 ] && echo "ROUTES=CLOUDFLARE-WS->$CF_PORT_STATE"; echo "RUNTIME_FINGERPRINT=$FINGERPRINT"; echo "NODES=$EXPECTED"
