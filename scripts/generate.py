...
if CF_CONFIG_PRESENT and not CF_ENABLED: CF_INVALID_REASON = "incomplete Cloudflare variable set; node 4 disabled"
if CF_ENABLED:
...
runtime = {"schema": 24, "build": "stable-node5-grpc-reality-v5", "architecture": "single-8080-router-plus-node5-grpc-and-optional-cloudflare-tunnel", "cloudflare": {"enabled": CF_ENABLED, "token_configured": bool(CF_TOKEN), "tunnel_id_configured": bool(CF_ID), "public_hostname": CF_HOST if CF_ENABLED else "", "origin_service": CF_ORIGIN if CF_ENABLED else "", "origin_service_input_present": bool(CF_ORIGIN_RAW), "ws_port": CF_PORT if CF_ENABLED else None, "ws_path": CF_PATH if CF_ENABLED else "", "validation_error": CF_INVALID_REASON}, "nodes": {"count": NODE_COUNT, "distribution": {"01": "domain-xhttp-tls", "02": "raw-reality-vision", "03": "xhttp-reality", "05": "grpc-reality", **({"04": "cloudflare-ws-tls"} if CF_ENABLED else {})}}, "application_port": APP_PORT, "public_domain": PUBLIC_DOMAIN, "tcp_proxy": {"domain": TCP_HOST, "port": TCP_PORT, "application_port": APP_PORT}, "railway_networking": {"source": "current-deployment-environment", "authoritative": True, "state": networking_state, "previous_public_domain": previous_public, "current_public_domain": PUBLIC_DOMAIN, "previous_tcp_proxy": previous_tcp, "current_tcp_proxy": current_tcp}, "routes": {"domain_xhttp_tls": {"port": 10086}, "raw_reality_vision": {"sni": RAW_SNI, "port": 10087, "short_id": ids[0]}, "xhttp_reality": {"sni": XHTTP_SNI, "port": 10088, "short_id": ids[1]}, "grpc_reality": {"sni": GRPC_SNI, "port": 10089, "short_id": ids[2], "service_name": GRPC_SERVICE_NAME}, **({"cloudflare_ws_tls": {"host": CF_HOST, "port": CF_PORT, "path": CF_PATH}} if CF_ENABLED else {})}}
...
manifest = {"schema": 24, "build": "stable-node5-grpc-reality-v5", "node_count": NODE_COUNT, "application_port": APP_PORT, "cloudflare_ws_enabled": CF_ENABLED, "distribution": runtime["nodes"]["distribution"], "runtime_fingerprint": runtime["fingerprint"], "railway_networking_source": "current-deployment-environment", "railway_networking_authoritative": True, "railway_networking_state": networking_state}
(D / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
print("RELEASE=stable-node5-grpc-reality-v5", flush=True); print(f"RUNTIME_FINGERPRINT={runtime['fingerprint']}", flush=True); print("RAILWAY_NETWORKING_SOURCE=current-deployment-environment", flush=True); print("RAILWAY_NETWORKING_AUTHORITATIVE=true", flush=True); print(f"RAILWAY_CURRENT_PUBLIC={PUBLIC_DOMAIN}", flush=True); print(f"RAILWAY_CURRENT_TCP={current_tcp}", flush=True); print(f"CLOUDFLARE_WS={'enabled' if CF_ENABLED else 'disabled'}", flush=True); print(f"SUBSCRIPTION_INVARIANT={NODE_COUNT}", flush=True); print(f"DOMAIN {PUBLIC_DOMAIN}:443 -> 8080 -> 10086 XHTTP TLS", flush=True); print(f"TCP {TCP_HOST}:{TCP_PORT} -> 8080 -> {RAW_SNI} -> 10087 RAW REALITY Vision", flush=True); print(f"TCP {TCP_HOST}:{TCP_PORT} -> 8080 -> {XHTTP_SNI} -> 10088 XHTTP REALITY", flush=True); print(f"TCP {TCP_HOST}:{TCP_PORT} -> 8080 -> {GRPC_SNI} -> 10089 gRPC REALITY serviceName={GRPC_SERVICE_NAME}", flush=True)
