...
if not TCP_HOST or not TCP_PORT_RAW: raise SystemExit("FATAL: Railway TCP Proxy unavailable")
...
if any(cf_vars) and not all(cf_vars): raise SystemExit("FATAL: incomplete Cloudflare configuration; provide all 6 variables or remove all Cloudflare variables")
...
if CF_ENABLED:
    ...
    if not 1 <= CF_PORT <= 65535: raise SystemExit("FATAL: Cloudflare XHTTP port outside 1-65535")
    if CF_PORT in (8080,10086,10087,10088,10089): raise SystemExit("FATAL: Cloudflare XHTTP port conflicts with internal port")
...
NODE_ORDER = ["railway-xhttp-tls", "raw-reality-vision", "xhttp-reality", "grpc-reality"] + (["cloudflare-xhttp-tls"] if CF_ENABLED else [])
NODE_DISTRIBUTION = {str(i + 1).zfill(2): name for i, name in enumerate(NODE_ORDER)}
...
runtime = {"schema":27,"build":"fix-5node-xhttp-cloudflare-v3","architecture":"fixed-node-order-1-railway-xhttp-2-raw-reality-3-xhttp-reality-4-grpc-reality-5-cloudflare-xhttp-tls-optional","cloudflare":{"enabled":CF_ENABLED,"transport":"xhttp","public_tls":True,"origin_protocol":"http","token_configured":bool(CF_TOKEN),"tunnel_id_configured":bool(CF_ID),"public_hostname":CF_HOST if CF_ENABLED else "","origin_service":CF_ORIGIN if CF_ENABLED else "","xhttp_port":CF_PORT if CF_ENABLED else None,"xhttp_path":CF_PATH if CF_ENABLED else ""},"nodes":{"count":NODE_COUNT,"distribution":NODE_DISTRIBUTION},"application_port":APP_PORT,"public_domain":PUBLIC_DOMAIN,"tcp_proxy":{"domain":TCP_HOST,"port":TCP_PORT,"application_port":APP_PORT,"expected_target":APP_PORT},"railway_networking":{"source":"current-deployment-environment","authoritative":True,"state":state,"previous_public_domain":prev_public,"current_public_domain":PUBLIC_DOMAIN,"previous_tcp_proxy":prev_tcp,"current_tcp_proxy":current_tcp},"routes":{"domain_xhttp_tls":{"port":10086},"raw_reality_vision":{"sni":RAW_SNI,"port":10087,"short_id":ids[0]},"xhttp_reality":{"sni":XHTTP_SNI,"port":10088,"short_id":ids[1]},"grpc_reality":{"sni":GRPC_SNI,"port":10089,"short_id":ids[2],"service_name":GRPC_SERVICE_NAME},**({"cloudflare_xhttp_tls":{"host":CF_HOST,"port":CF_PORT,"path":CF_PATH,"origin":CF_ORIGIN}} if CF_ENABLED else {})}}
...
(D/"manifest.json").write_text(json.dumps({"schema":27,"build":"fix-5node-xhttp-cloudflare-v3","node_count":NODE_COUNT,"application_port":APP_PORT,"cloudflare_xhttp_enabled":CF_ENABLED,"distribution":NODE_DISTRIBUTION,"runtime_fingerprint":runtime["fingerprint"],"railway_networking_source":"current-deployment-environment","railway_networking_authoritative":True,"railway_networking_state":state,"tcp_proxy_expected_target":APP_PORT}, indent=2)+"\n")
print("RELEASE=fix-5node-xhttp-cloudflare-v3",flush=True); print(f"RUNTIME_FINGERPRINT={runtime['fingerprint']}",flush=True); print("RAILWAY_NETWORKING_SOURCE=current-deployment-environment",flush=True); print("RAILWAY_NETWORKING_AUTHORITATIVE=true",flush=True); print(f"RAILWAY_NETWORKING={state}",flush=True); print(f"RAILWAY_CURRENT_PUBLIC={PUBLIC_DOMAIN}",flush=True); print(f"RAILWAY_CURRENT_TCP={current_tcp}",flush=True); print("RAILWAY_TCP_PROXY_EXPECTED_TARGET=8080",flush=True); print(f"CLOUDFLARE_XHTTP={'enabled' if CF_ENABLED else 'disabled'}",flush=True); print(f"SUBSCRIPTION_INVARIANT={NODE_COUNT}",flush=True); print("NODE_ORDER=" + ",".join(f"{i+1}:{name}" for i,name in enumerate(NODE_ORDER)),flush=True); print(f"NODES={NODE_COUNT}",flush=True)
