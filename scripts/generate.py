...
CF_VARIABLES = (CF_TOKEN, CF_ID, CF_HOST, CF_ORIGIN_RAW, CF_PORT_RAW, CF_PATH); CF_CONFIG_PRESENT = any(CF_VARIABLES); CF_ENABLED = all(CF_VARIABLES); CF_PORT = None; CF_INVALID_REASON = ""; CF_ORIGIN = ""
if CF_CONFIG_PRESENT and not CF_ENABLED: CF_INVALID_REASON = "incomplete Cloudflare variable set; node 5 disabled"
if CF_ENABLED:
    try: CF_PORT = int(CF_PORT_RAW)
    except ValueError: CF_ENABLED = False; CF_INVALID_REASON = "WS_PORT is not an integer"
    if CF_ENABLED and not 1 <= CF_PORT <= 65535: CF_ENABLED = False; CF_INVALID_REASON = "WS_PORT outside 1-65535"
    if CF_ENABLED and CF_PORT == APP_PORT: CF_ENABLED = False; CF_INVALID_REASON = "WS_PORT must differ from GATEWAY_PORT 8080"
    if CF_ENABLED and CF_PORT in (10086, 10087, 10088, 10089): CF_ENABLED = False; CF_INVALID_REASON = "WS_PORT conflicts with an existing Xray inbound port"
    if CF_ENABLED and not re.fullmatch(r"[A-Za-z0-9.-]+", CF_HOST): CF_ENABLED = False; CF_INVALID_REASON = "CLOUDFLARE_PUBLIC_HOSTNAME invalid"
    if CF_ENABLED and not CF_PATH.startswith("/"): CF_ENABLED = False; CF_INVALID_REASON = "WS_PATH must start with /"
    if CF_ENABLED: CF_ORIGIN = f"http://127.0.0.1:{CF_PORT}"
...
# Fixed subscription order: Node 1 Railway XHTTP TLS, Node 2 RAW REALITY Vision,
# Node 3 XHTTP REALITY, Node 4 gRPC REALITY, Node 5 Cloudflare WS TLS.
lines = [
...
if CF_ENABLED:
    lines.append(link(CF_HOST, 443, {"encryption": "none", "security": "tls", "sni": CF_HOST, "fp": FP, "alpn": "http/1.1", "type": "ws", "host": CF_HOST, "path": CF_PATH}, "VLESS WS TLS · Cloudflare Tunnel"))
NODE_COUNT = len(lines)
if NODE_COUNT not in (4, 5): raise SystemExit(f"FATAL: invalid node count: {NODE_COUNT}")

previous_runtime = {}; previous_runtime_file = D / "runtime.json"
if previous_runtime_file.is_file():
    try: previous_runtime = json.loads(previous_runtime_file.read_text())
    except Exception: previous_runtime = {}
previous_public = str(previous_runtime.get("public_domain", "")); previous_tcp_state = previous_runtime.get("tcp_proxy", {}) or {}; previous_tcp = f"{previous_tcp_state.get('domain', '')}:{previous_tcp_state.get('port', '')}" if (previous_tcp_state.get("domain") or previous_tcp_state.get("port")) else ""; current_tcp = f"{TCP_HOST}:{TCP_PORT}"
networking_state = "initial" if not previous_runtime else ("unchanged" if previous_public == PUBLIC_DOMAIN and previous_tcp == current_tcp else "changed")
runtime = {"schema": 24, "build": "stable-node5-grpc-reality-v6", "architecture": "fixed-node-order-1-railway-xhttp-2-raw-reality-3-xhttp-reality-4-grpc-reality-5-cloudflare-ws", "cloudflare": {"enabled": CF_ENABLED, "token_configured": bool(CF_TOKEN), "tunnel_id_configured": bool(CF_ID), "public_hostname": CF_HOST if CF_ENABLED else "", "origin_service": CF_ORIGIN if CF_ENABLED else "", "origin_service_input_present": bool(CF_ORIGIN_RAW), "ws_port": CF_PORT if CF_ENABLED else None, "ws_path": CF_PATH if CF_ENABLED else "", "validation_error": CF_INVALID_REASON}, "nodes": {"count": NODE_COUNT, "distribution": {"01": "domain-xhttp-tls", "02": "raw-reality-vision", "03": "xhttp-reality", "04": "grpc-reality", **({"05": "cloudflare-ws-tls"} if CF_ENABLED else {})}}, "application_port": APP_PORT, "public_domain": PUBLIC_DOMAIN, "tcp_proxy": {"domain": TCP_HOST, "port": TCP_PORT, "application_port": APP_PORT}, "railway_networking": {"source": "current-deployment-environment", "authoritative": True, "state": networking_state, "previous_public_domain": previous_public, "current_public_domain": PUBLIC_DOMAIN, "previous_tcp_proxy": previous_tcp, "current_tcp_proxy": current_tcp}, "routes": {"domain_xhttp_tls": {"port": 10086}, "raw_reality_vision": {"sni": RAW_SNI, "port": 10087, "short_id": ids[0]}, "xhttp_reality": {"sni": XHTTP_SNI, "port": 10088, "short_id": ids[1]}, "grpc_reality": {"sni": GRPC_SNI, "port": 10089, "short_id": ids[2], "service_name": GRPC_SERVICE_NAME}, **({"cloudflare_ws_tls": {"host": CF_HOST, "port": CF_PORT, "path": CF_PATH}} if CF_ENABLED else {})}}
...
manifest = {"schema": 24, "build": "stable-node5-grpc-reality-v6", "node_count": NODE_COUNT, "application_port": APP_PORT, "cloudflare_ws_enabled": CF_ENABLED, "distribution": runtime["nodes"]["distribution"], "runtime_fingerprint": runtime["fingerprint"], "railway_networking_source": "current-deployment-environment", "railway_networking_authoritative": True, "railway_networking_state": networking_state}
(D / "manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
print("RELEASE=stable-node5-grpc-reality-v6", flush=True); print(f"RUNTIME_FINGERPRINT={runtime['fingerprint']}", flush=True); print("RAILWAY_NETWORKING_SOURCE=current-deployment-environment", flush=True); print("RAILWAY_NETWORKING_AUTHORITATIVE=true", flush=True); print(f"RAILWAY_CURRENT_PUBLIC={PUBLIC_DOMAIN}", flush=True); print(f"RAILWAY_CURRENT_TCP={current_tcp}", flush=True); print(f"CLOUDFLARE_WS={'enabled' if CF_ENABLED else 'disabled'}", flush=True); print(f"SUBSCRIPTION_INVARIANT={NODE_COUNT}", flush=True); print(f"NODE_ORDER=1:railway-xhttp-tls,2:raw-reality-vision,3:xhttp-reality,4:grpc-reality,5:cloudflare-ws-tls", flush=True); print(f"DOMAIN {PUBLIC_DOMAIN}:443 -> 8080 -> 10086 XHTTP TLS", flush=True); print(f"TCP {TCP_HOST}:{TCP_PORT} -> 8080 -> {RAW_SNI} -> 10087 RAW REALITY Vision", flush=True); print(f"TCP {TCP_HOST}:{TCP_PORT} -> 8080 -> {XHTTP_SNI} -> 10088 XHTTP REALITY", flush=True); print(f"TCP {TCP_HOST}:{TCP_PORT} -> 8080 -> {GRPC_SNI} -> 10089 gRPC REALITY serviceName={GRPC_SERVICE_NAME}", flush=True)
if CF_ENABLED: print(f"CF {CF_HOST}:443 -> tunnel -> {CF_ORIGIN} -> {CF_PORT} WS TLS", flush=True)
