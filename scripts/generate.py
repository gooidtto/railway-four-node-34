...
if CF_ENABLED:
    ...
    if not 1 <= CF_PORT <= 65535: raise SystemExit("FATAL: WS_PORT outside 1-65535")
    if CF_PORT in (8080,10086,10087,10088,10089): raise SystemExit("FATAL: WS_PORT conflicts with internal port")
    if not re.fullmatch(r"[A-Za-z0-9.-]+", CF_HOST): raise SystemExit("FATAL: invalid Cloudflare hostname")
    if not CF_PATH.startswith("/"): raise SystemExit("FATAL: WS_PATH must start with /")
...
(D/"manifest.json").write_text(json.dumps({"schema":25,"build":"fix-5node-lifecycle-v1","node_count":NODE_COUNT,"application_port":APP_PORT,"cloudflare_ws_enabled":CF_ENABLED,"distribution":runtime["nodes"]["distribution"],"runtime_fingerprint":runtime["fingerprint"],"railway_networking_source":"current-deployment-environment","railway_networking_authoritative":True,"railway_networking_state":state}, indent=2)+"\n")
print("RELEASE=fix-5node-lifecycle-v1",flush=True); print(f"RUNTIME_FINGERPRINT={runtime['fingerprint']}",flush=True); print("RAILWAY_NETWORKING_SOURCE=current-deployment-environment",flush=True); print("RAILWAY_NETWORKING_AUTHORITATIVE=true",flush=True); print(f"RAILWAY_NETWORKING={state}",flush=True); print(f"RAILWAY_CURRENT_PUBLIC={PUBLIC_DOMAIN}",flush=True); print(f"RAILWAY_CURRENT_TCP={current_tcp}",flush=True); print(f"CLOUDFLARE_WS={'enabled' if CF_ENABLED else 'disabled'}",flush=True); print(f"SUBSCRIPTION_INVARIANT={NODE_COUNT}",flush=True); print("NODE_ORDER=1:railway-xhttp-tls,2:raw-reality-vision,3:xhttp-reality,4:grpc-reality,5:cloudflare-ws-tls",flush=True); print(f"NODES={NODE_COUNT}",flush=True)
